...
*This issue was automatically created by the Enhanced Monitor V3*`;
    ...
    async initialize() {
        ...
        // Get bot username early to ensure it's available for filtering
    ...
    async loadProcessedComments() {
        try {
            ...
            // Clean up old processed comments (older than 7 days) to prevent unbounded growth
            const oneWeekAgo = Date.now() - (7 * 24 * 60 * 60 * 1000);
            const recentComments = comments.filter(id => {
                // Comment IDs are generally incrementing, so newer IDs are larger
                // This is a heuristic but helps prevent the set from growing infinitely
                return id > oneWeekAgo / 1000; // Rough approximation
            });
            
            if (recentComments.length < comments.length) {
                this.processedComments = new Set(recentComments);
                await this.saveProcessedComments();
                await this.log(`Cleaned up ${comments.length - recentComments.length} old processed comments`);
            }
        } catch (error) {
        ...
        try {
            ...
            for (const comment of comments) {
                ...
                const isAIAgentComment = comment.body.includes('⚙️ GitHub Automation | Powered by System') ||
                                       ...
                                       comment.body.includes('🤖 **Claude Response**') ||
                                       comment.body.includes('🤖 **Claude\'s Response') ||
                                       comment.body.includes('🤖 **Follow-up Response**') ||
                                       comment.body.includes('❌ **Error Processing') ||
                                       comment.body.includes('Processing directive from @');
